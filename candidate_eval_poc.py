...
#
# Proof-of-concept: cross-encoder for candidate evaluation (Q, C -> 4 labels)
...
def generate_forbidden_qualification() -> Dict[str, str]:
    ...
    forbidden_qual_templates = [
        ...
        "Looking for a young candidate under 30 years old.",
        "Only male applicants will be considered for this role.",
        "Seeking a specific ethnic background for cultural fit.",
...
def generate_synthetic_example() -> Dict[str, str]:
    ...
    FORBIDDEN now means: the qualification itself is "bad" (biased / discriminatory).
    For MATCH / PARTIAL_MATCH / NOT_MATCH, qualifications are normal and the label
    depends on how well the candidate fits.
    """
    # Randomly choose label first so we can special-case FORBIDDEN.
    patterns = ["MATCH", "PARTIAL_MATCH", "NOT_MATCH", "FORBIDDEN"]
    label = random.choice(patterns)

    # If the rubric says this should be forbidden, we generate a biased/discriminatory qualification.
    if label == "FORBIDDEN":
        return generate_forbidden_qualification()

    # For non-FORBIDDEN, we generate normal qualifications and vary candidate fit.
